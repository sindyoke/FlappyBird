...
public class GameEngine {
    ...
    public GameEngine() {
        ...
        tubes = new ArrayList<>();
        random = new Random();
        int numberOfTubes = AppConstants.numberOfTubes;
    ...
    public void updateAndDrawTubes(Canvas canvas) {
        if (gameState == 1) {
            if ((tubes.get(scoringTube).getTubeX() < bird.getX() + AppConstants.getBitmapBank().getBirdWidth())
                ...
                GameOverActivity.start(context, score);
            ...
            for (int i = 0; i < AppConstants.numberOfTubes; i++) {
                ...
                if (tubes.get(i).getTubeColor() == 0) {
                    ...
                    canvas.drawBitmap(AppConstants.getBitmapBank().getTubeTopRed(), tubes.get(i).getTubeX(), tubes.get(i).getTopTubeY(), null);
                    canvas.drawBitmap(AppConstants.getBitmapBank().getTubeBottomRed(), tubes.get(i).getTubeX(), tubes.get(i).getBottomTubeY(), null);
                } else {
                    canvas.drawBitmap(AppConstants.getBitmapBank().getTubeTopBlue(), tubes.get(i).getTubeX(), tubes.get(i).getTopTubeY(), null);
                    canvas.drawBitmap(AppConstants.getBitmapBank().getTubeBottomBlue(), tubes.get(i).getTubeX(), tubes.get(i).getBottomTubeY(), null);
                }
            }
...
//                Log.i("GameEngine", "bird velocity is " + bird.getVelocity() + ", birdY is " + bird.getY());
            }
        }
